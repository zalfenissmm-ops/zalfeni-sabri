...
|---------|-----------|-------|
| **`InpPreset`** | **Balanced** | **الوضع — يتحكّم في كل الشروط دفعة وحدة (شوف تحت)** |
...
### الأوضاع (Presets) — إذا ما تشوفش زونات برشا

المؤشر يجمع 5 شروط مع بعض، وكل شرط ينقّص من عدد الزونات. `InpPreset` يخلّيك
تبدّل الحزمة الكل بإعداد واحد:

| الوضع | الشروط | Displacement | Swing | النتيجة (اختبار على 900 شمعة) |
|-------|--------|--------------|-------|-------------------------------|
| **Strict** | sweep + disp + FVG إجباريين | ×2.0، أجسام 55%، ≤4 شموع | 3 | **11** زون |
| **Balanced** (افتراضي) | sweep + disp إجباريين، FVG = نقاط | ×1.5، أجسام 45%، ≤6 شموع | 2 | **48** زون |
| **Loose** | disp برك | ×1.2، أجسام 40%، ≤8 شموع | 2 | **83** زون |
| **Custom** | يقرا الإعدادات اللي تحتو كيما هي | — | — | — |

> علاش الـ FVG ولّى نقطة قوة موش شرط رفض في Balanced؟ خاطر كيما قلت روحك:
> «بدون FVG الـ OB **ضعيف**» — ضعيف موش باطل. توّا يتعرض بتقييم أنقص (B/C)
> عوض ما يتلغى. نفس الشيء للـ mitigated: يتعرض بلون باهت وإطار منقّط.

**إذا مازال قليل:** نقّص `InpSwingWindow` لـ 2 (ستركتشر أصغر = كسور أكثر)،
زيد `InpMaxOB`، وشغّل `InpPrintLog=true` باش تشوف في اللوج علاش تلغات كل زون
(الغالب يكون "displacement ضعيف").
