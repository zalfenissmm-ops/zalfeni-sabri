...
## مؤشر MT5 — MT5/ZalfeniSignals.mq5

مؤشر لميتاتريدر 5 يعطي **إشارات بيع وشراء مباشرة على الشارت** (أسهم فوق/تحت الشمعة)،
بنفس منطق `trend_identifier.py` بالضبط: ثلاث إشارات متكاملة بدل مؤشر واحد.

- **السهم الأزرق ↑** = إشارة شراء، **السهم الأحمر ↓** = إشارة بيع.
- الإشارة تطلع **كان يتقلب الاتجاه** (مش على كل شمعة): سهم واحد لكل دورة.
- الشرط: توافق السعر مع الـ EMA + بنية القمم/القيعان، و **ADX فوق الحد الأدنى** —
  كيف السوق عرضي (ADX ضعيف) المؤشر يسكت ما يعطيش إشارات.
- تنبيه فوري (popup) + إشعار للموبايل + صوت، مع **SL و TP مقترحين** محسوبين من آخر قمة/قاع + ATR.
- لوحة معلومات على الشارت: الاتجاه، القوة (ADX)، موقع السعر من الـ EMA، البنية، وآخر إشارة.

### التركيب

1. افتح ميتاتريدر 5 → `File` → `Open Data Folder`.
2. حط الملف `ZalfeniSignals.mq5` في `MQL5/Indicators/`.
3. افتح MetaEditor (F4) → اختار الملف → `Compile` (F7).
4. ارجع للترمينال → `Navigator` → `Indicators` → اسحب **ZalfeniSignals** على الشارت.

### الإعدادات

| الإعداد | الافتراضي | الشرح |
|---|---|---|
| `InpEmaPeriod` | 50 | فترة الـ EMA (موقع السعر) |
| `InpAdxPeriod` | 14 | فترة الـ ADX (قوة الترند) |
| `InpAdxMinStrength` | 20 | أقل ADX تتقبل بيه إشارة — زيدها لـ 25 تنقّص إشارات السوق العرضي |
| `InpSwingWindow` | 5 | نافذة القمم/القيعان (عدد الشموع من كل جهة) |
| `InpResetOnNeutral` | false | يسمح بإشارة ثانية في نفس الاتجاه بعد ما الترند يولّي محايد |
| `InpClosedBarsOnly` | true | الإشارة على الشموع المغلوقة برك (بلا repaint) |
| `InpAtrArrowGap` | 0.6 | بُعد السهم على الشمعة (× ATR) |
| `InpAtrStopPad` | 0.5 | هامش الستوب تحت/فوق آخر قاع/قمة (× ATR) |
| `InpRiskReward` | 2.0 | الهدف = R:R × المخاطرة |
| `InpShowLevelLines` | true | خطوط الدخول/SL/TP لآخر إشارة |
| `InpAlertPopup` / `InpAlertPush` / `InpAlertSound` | true / false / false | نوع التنبيه |

### مهم

- **بلا repaint**، أما فمّا تأخير طبيعي: القمة/القاع ما يتأكدش كان بعد `InpSwingWindow` شمعة،
  والسهم يتثبّت وقت الشمعة تسكّر. يعني تشوف الإشارة في افتتاح الشمعة اللي بعدها.
- المؤشر يعطي **قراءة اتجاه**، موش أمر تداول. استعملها مع إدارة رأس المال متاعك،
  والـ SL/TP المقترحين هوما نقطة بداية لازمك تعدّلهم حسب السوق والسبريد.
