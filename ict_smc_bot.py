...
# ============================================================================
def _load_secrets(path="telegram.json"):
    """Credentials come from env vars first, else from a local telegram.json
    (git-ignored). Never hard-code them in this file - it gets shared."""
    try:
        with open(path) as f:
            cfg = json.load(f)
    except Exception:                            # noqa: BLE001
        cfg = {}
    return (os.environ.get("TELEGRAM_BOT_TOKEN") or cfg.get("bot_token") or "",
            os.environ.get("TELEGRAM_CHAT_ID") or cfg.get("chat_id") or "")
...
class Params:
    ...
    retrace_window: int = 48      # entry must happen within N M5 candles after the MSS (0 = unlimited)
...
def find_all_sweeps(m15, p: Params, use_lookback=True):
    ...
    found = {}
    # A fractal at si is only KNOWN at si + sw (it needs sw candles after it),
    # so a sweep of it cannot be acted on before then. And once price CLOSES
    # through a level that level is broken, not swept - it stops being liquidity.
    for (si, level) in lows:
        for j in range(si + p.sw_m15, n):
            c = m15[j]
            if c.c < level:                       # real breakdown -> level is dead
                break
            if c.l < level:                       # dipped under, closed back above
                if j >= floor_idx:
    ...
    for (si, level) in highs:
        for j in range(si + p.sw_m15, n):
            c = m15[j]
            if c.c > level:                       # real breakout -> level is dead
                break
            if c.h > level:
                if j >= floor_idx:
...
def detect_mss(m5, start_idx, side, p: Params, m5_swings=None):
    ...
    # The reaction extreme is the low/high made BETWEEN the sweep and the break.
    # It must never be taken from the whole window: that would look into the
    # future and park the stop under a low that has not happened yet.
    if side == "bull":
        for (hi, pr) in [(i, v) for (i, v) in highs if start_idx < i < end]:
            for k in range(hi + 1, end):
                if m5[k].c > pr:
                    rl_idx = min(range(start_idx, k), key=lambda q: m5[q].l)
                    return {"ref_idx": hi, "break_idx": k, "ext_idx": rl_idx}
        return None
    else:
        for (li, pr) in [(i, v) for (i, v) in lows if start_idx < i < end]:
            for k in range(li + 1, end):
                if m5[k].c < pr:
                    rh_idx = max(range(start_idx, k), key=lambda q: m5[q].h)
...
def _why_no_mss(m5, start_idx, side, p: Params, m5_swings):
    ...
    if side == "bull":
        cands = [i for (i, _) in highs if start_idx < i < end]
    else:
        cands = [i for (i, _) in lows if start_idx < i < end]
...
def _eval_one(sweep, h1, m15, m5, m5_swings, atr_arr, m5_times, p) -> Decision:
    ...
    r_start = max(mss["break_idx"] + 1, fvg["idx"] + 2)
    if r_start >= len(m5):
        return Decision(False, f"[{dir_txt}] FVG not complete yet - waiting for it to close.", ctx)
    if p.retrace_window and (len(m5) - 1 - r_start) > p.retrace_window:
        return Decision(False, f"[{dir_txt}] Setup expired - no retracement within the window.", ctx)
    react, sl, entry = sl_entry(m5, start_idx, mss, atr_arr, side, fvg, p.entry_mode)
    for k in range(r_start, len(m5)):
...
def _resolve_on_m1(tr: Trade, m1, m1_times, entry_open, max_hold=0):
    ...
    A minute that OPENS beyond the stop is filled at that open, not at the stop -
    that is what a weekend or news gap really costs; TP gets no matching bonus.
    If max_hold seconds pass without TP/SL the trade closes at market and is
    marked TIMEOUT, kept out of the win rate so scratches cannot pad it."""
    ...
    for j in range(i, len(m1)):
        ...
        if tr.side == "bull":
            if b.l <= tr.sl:
                tr.result, tr.exit_price, tr.t_exit = "LOSS", min(tr.sl, b.o), b.t; break
            if b.h >= tr.tp:
                tr.result, tr.exit_price, tr.t_exit = "WIN", tr.tp, b.t; break
        else:
            if b.h >= tr.sl:
                tr.result, tr.exit_price, tr.t_exit = "LOSS", max(tr.sl, b.o), b.t; break
            if b.l <= tr.tp:
                tr.result, tr.exit_price, tr.t_exit = "WIN", tr.tp, b.t; break
        if max_hold and (b.t - fill_t) >= max_hold:
            tr.result, tr.exit_price, tr.t_exit = "TIMEOUT", b.c, b.t
            break
    if tr.result in ("WIN", "LOSS", "TIMEOUT"):
...
STAGES = [
    ...
    ("7_no_retrace", "no retrace in window"),
...
def analyze(h1, m15, m5, m1, p: Params, days: int,
    ...
    for sweep in sweeps:
        ...
        r_start = max(mss["break_idx"] + 1, fvg["idx"] + 2)
        r_end = min(len(m5), r_start + p.retrace_window) if p.retrace_window else len(m5)
...
def _equity_stats(trades, balance, risk_pct):
    ...
    eq = peak = balance
    max_dd = 0.0
    for t in sorted((x for x in trades if x.result in ("WIN", "LOSS", "TIMEOUT")), key=lambda x: x.t_entry):
...
def _print_analysis(p: Params, days: int, st: Counter, trades, m1_used, entry_tf="M5"):
    ...
    touts = [t for t in trades if t.result == "TIMEOUT"]
    ...
    if touts:
        print(f"  Time-stopped    : {len(touts)}  (neither TP nor SL - excluded from win rate)")
    if opens:
        print(f"  Unresolved      : {len(opens)}  (no M1 data / still open at end)")
    if closed:
        print(f"  Win rate        : {100.0 * len(wins) / closed:.1f}%   (TP vs SL only)")
        print(f"  Net R           : {sum(t.r_mult for t in wins + losses + touts):+.2f} R"
              + (f"  (incl. {sum(t.r_mult for t in touts):+.2f} R from time-stops)" if touts else ""))
    ...
    for n_, t in enumerate(trades, 1):
        ...
        print(f"  #{n_:<3} {side} [{t.result:7s}]  RR=1:{t.rr:.2f}  R={t.r_mult:+.2f}")
...
def build_params(args) -> Params:
    ...
    for f_ in ("symbol", "sweep_lookback", "mss_window", "atr_period",
               "disp_atr_mult", "disp_body_ratio", "rr_min", "sw_h1", "sw_m15", "sw_m5", "retrace_window",
...
def main():
    ...
    ap.add_argument("--entry", dest="entry_mode", choices=["ce", "edge", "far"], help="entry inside FVG (default edge)")
    ...
    ap.add_argument("--retrace-window", type=int, dest="retrace_window",
                    help="max M5 candles between MSS and entry (0 = unlimited, default 48)")
    ...
    feed = MT5Feed(p)
    feed.connect()
    telegram_send(f"ICT/SMC Bot started ({', '.join(symbols)}). "
                  f"Telegram connected - signals will arrive here.")
    try:
