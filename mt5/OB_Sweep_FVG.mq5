...
input group             "=== Display / العرض ==="
input bool   InpShowValid       = true;  // Show valid order blocks
input bool   InpShowInvalid     = true;  // Show invalid order blocks
input bool   InpShowFVG         = true;  // Show imbalance / FVG box
input bool   InpShowSweepLine   = true;  // Show swept liquidity level
input bool   InpShowLabels      = true;  // Show text labels
input bool   InpShowArrows      = true;  // Show arrow on the OB candle
input bool   InpHideBroken      = false; // Delete zone once price breaks it
input int    InpExtendBars      = 8;     // Extend zones N bars to the right
input int    InpMaxZones        = 30;    // Max zones kept on chart

input group             "=== Colours / الألوان ==="
input color  InpBullOBColor     = C'0,110,75';    // Bullish OB
input color  InpBearOBColor     = C'150,35,45';   // Bearish OB
input color  InpInvalidColor    = C'85,85,85';    // Invalid / broken OB
input color  InpFVGColor        = C'60,70,130';   // Imbalance / FVG
input color  InpSweepColor      = clrSilver;      // Liquidity sweep level
input color  InpTextColor       = clrWhiteSmoke;  // Labels
...
struct SZone
  {
   ...
   double            fvgBottom;
   double            sweepLevel;
   datetime          tSweepFrom;  // time of the bar that created the swept level
...
void ObjRect(const string name,datetime t1,double p1,datetime t2,double p2,color clr)
  {
...
//+------------------------------------------------------------------+
void DrawZone(const int i,const datetime tRight)
  {
   SZone z = g_zones[i];
   if(z.valid && !InpShowValid)     return;
   if(!z.valid && !InpShowInvalid)  return;
...
//--- order block box
   ObjRect(base+"OB",z.tStart,z.obTop,tEnd,z.obBottom,obc);

//--- imbalance / FVG box
   if(InpShowFVG && z.hasFVG && z.valid)
      ObjRect(base+"FVG",z.tStart,z.fvgTop,tEnd,z.fvgBottom,InpFVGColor);
...
//--- labels
   if(InpShowLabels)
     {
      string txt = z.valid ? "Valid OB" : "InValid OB";
      if(z.state==ZONE_BROKEN)
         txt = "OB Broken";
      else
         if(z.state==ZONE_TOUCHED && z.valid)
            txt = "Valid OB (tested)";
      ObjLabelText(base+"T",tEnd,(z.obTop+z.obBottom)/2.0,txt+" ",InpTextColor);

      if(InpShowFVG && z.hasFVG && z.valid)
         ObjLabelText(base+"TF",tEnd,(z.fvgTop+z.fvgBottom)/2.0,"FVG ",InpTextColor);
     }
  }

//+------------------------------------------------------------------+
//| Remove chart objects of one zone                                 |
//+------------------------------------------------------------------+
void DeleteZoneObjects(const int i)
  {
   ObjectsDeleteAll(0,ZoneBase(g_zones[i]));
  }

//+------------------------------------------------------------------+
//| Append a zone, keeping the list bounded                          |
//+------------------------------------------------------------------+
...
//--- keep only the newest InpMaxZones zones
   int maxZ = MathMax(1,InpMaxZones);
   while(ArraySize(g_zones)>maxZ)
     {
      DeleteZoneObjects(0);
      ...
      for(int b=MathMax(0,g_zones[i].checked); b<=rates_total-2; b++)
        {
         ...
         bool inside = (low[b]<=g_zones[i].obTop && high[b]>=g_zones[i].obBottom);
         if(inside && g_zones[i].state==ZONE_FRESH)
