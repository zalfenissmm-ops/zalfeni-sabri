...
//--- trade outcome codes (always compare with ==, never with >)
#define RES_RUNNING   0
#define RES_TARGET    1
#define RES_STOP     (-1)
...
struct TradeSetup
  {
   ...
   int      result;     // RES_TARGET / RES_STOP / RES_RUNNING
   int      fillIdx;    // bar where the order at the level was filled
   int      resultIdx;
   datetime entryTime;
  };
...
//--- entry: after the sweep candle closes, an order rests AT the swept level
//--- (buy stop above it / sell stop below it). It fills when price trades
//--- back through the level within InpReclaimBars bars - which is exactly
//--- what the video does: the entry sits on the level, price comes back
//--- through it, the trade is on.
   ...
   double invalid = bullish ? g_fvg[fi].bottom-buffer : g_fvg[fi].top+buffer;

   if((bullish && close[k]>lqLevel) || (!bullish && close[k]<lqLevel))
     {
      //--- swept and reclaimed inside the same candle: the order could not
      //--- have been resting yet, so the honest fill is that candle's close
      r=k;
      entry=close[k];
     }
   else
         ...
         if(bullish)
           {
            ...
            if(low[i]<invalid) break;                 // gap blown, setup dead
            if(high[i]>=lqLevel) { r=i; break; }      // stop order filled at the level
           }
         else
           {
            if(high[i]>ext) ext=high[i];
            if(high[i]>invalid) break;
            if(low[i]<=lqLevel) { r=i; break; }
           }
        }
     }
   if(r<0)
...
//--- outcome from the fill bar onwards (stop checked first, conservative)
   int res=RES_RUNNING, resIdx=-1;
   for(int i=r; i<n; i++)
     {
      if(bullish)
        {
         if(low[i]<=sl)  { res=RES_STOP;   resIdx=i; break; }
         if(high[i]>=tp) { res=RES_TARGET; resIdx=i; break; }
        }
      else
        {
         if(high[i]>=sl) { res=RES_STOP;   resIdx=i; break; }
         if(low[i]<=tp)  { res=RES_TARGET; resIdx=i; break; }
        }
     }
